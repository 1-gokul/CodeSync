...
CodeSync is a native Python desktop application that automates local workspace synchronization with remote GitHub repositories. It combines local Git version control and the GitHub API into a single dashboard, so you don't run `git add`, `git commit`, and `git push` by hand every time.
...
* **Auto-generated Commit Messages**: Analyzes modified files and suggests a relevant commit message.
...
* **Background Scheduler**: Automates syncing at regular intervals using a non-blocking background thread.
...
│   └── helpers.py              # Commit message generator
│
├── tests/
│   ├── test_git.py
│   ├── test_github.py
